...
const { ITEMS, MOBS, WORLD, STARTER, xpForLevel, statsForLevel } = require('./gamedata');
...
function playerAttackMob(player, mobId, io) {
  ...
  if (dist2(player.pos, mob.pos) > 9) return;          // out of range
...
function playerAttackPlayer(attacker, targetId, io) {
  ...
  if (dist2(attacker.pos, target.pos) > 9) return;
...
async function authenticateOrRegister({ username, password, charClass, gender }) {
  ...
  // New account → must supply class + gender.
  if (!charClass || !gender) {
  ...
  record = await PlayerStore.create({
    username, charClass, gender, passwordHash,
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('auth:check', async ({ username, password }, ack) => {
      try {
        ...
        return ack && ack({
          ok: true, exists: true,
          character: { charClass: record.charClass, gender: record.gender, level: record.level },
        });
      } catch (e) {
    ...
    socket.on('join', async ({ username, password, charClass, gender }, ack) => {
      try {
        if (!username || !/^[a-zA-Z0-9_]{2,16}$/.test(username)) {
          return ack && ack({ ok: false, error: 'Username must be 2–16 letters, numbers, or underscores.' });
        }
        ...
        const auth = await authenticateOrRegister({ username, password, charClass, gender });
        if (!auth.ok) {
