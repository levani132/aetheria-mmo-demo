...
export function loadElfMageFemale() {
  ...
  pendingPromise = Promise.all([load(CHAR_URL), load(ANIM_URL)])
    .then(([charGltf, animGltf]) => {
      ...
      // Strip the Hips position track from every clip. The Meshy clips were
      // authored with root motion — the Hips bone translates forward each loop
      // of Running, sideways during casts, and bobs slightly during Idle. Game
      // logic moves the whole rig Group via WASD, so leaving root motion in
      // the clips would compound: the rig drifts ahead of its "world" position
      // and the visual desynchronises from selfMesh.position.
