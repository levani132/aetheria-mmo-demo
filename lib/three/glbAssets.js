...
export function loadElfMageFemale() {
  ...
  pendingPromise = Promise.all([load(CHAR_URL), load(ANIM_URL)])
    .then(([charGltf, animGltf]) => {
      ...
      // NB: we leave the Hips.position track intact. Stripping it stops the
      // horizontal "runs ahead of its world position" drift, but it also kills
      // the vertical bob that the run clip uses to plant/lift the feet — so
      // the feet end up clipping the ground or floating mid-air. Instead, the
      // animation loop in characters.js re-anchors Hips.x and Hips.z to their
      // bind values after each mixer.update, which kills the drift while
      // preserving the vertical motion that keeps feet on the ground.
