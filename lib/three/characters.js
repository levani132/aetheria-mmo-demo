...
function attachElfMageGlb(grp) {
  ...
  // The 5.5 m/s player speed is jogging pace, so Running drives movement.
  u.idleAction = rig.actions.Idle_3 || null;
  u.moveAction = rig.actions.Running || rig.actions.Walking_Woman || rig.actions.Walking || null;
  u.castClips = ['mage_soell_cast', 'mage_soell_cast_1', 'mage_soell_cast_4']
  ...
  // Start idle. Idle_3 loops continuously so she has subtle breathing motion
  // (its hip-position root motion was already stripped in glbAssets so it stays
  // in place).
  if (u.idleAction) {
...
function startGlbMove(u) {
  if (u.idleAction) u.idleAction.stop();
  if (u.moveAction) {
    u.moveAction.reset();
...
// Go to the looping idle clip. Used when not moving and not casting. The Idle
// clip's Hips position track is stripped (in glbAssets) so she doesn't drift.
...
function triggerGlbCast(grp) {
  ...
  // mid-cast), explicitly stop it — otherwise both AnimationActions would
  // contribute to the rig's pose at once.
  if (u.castAction && u.castAction !== cast) u.castAction.stop();
  // Stop move and idle while the cast plays — the cast clip animates the
  // full body and would crossfade weirdly with running legs or breathing.
  if (u.moveAction) u.moveAction.stop();
  if (u.idleAction) u.idleAction.stop();
  u.moveActive = false;
